...
class member_consumption(osv.osv):
    ...
    _columns = {
        ...
        "total_paid_fee" : fields.float("total_paid_fee",digits_compute = dp.get_precision('ktv_fee'),help = "本次付款金额合计,该字段用于积分"),
        "paid_fee" : fields.float("paid_fee",digits_compute = dp.get_precision('ktv_fee'),help = "本次使用会员卡付款金额"),
        "before_paid_balance" : fields.float("before_paid_banalce",readonly = True,digits_compute = dp.get_precision('ktv_fee'),help = "付款前卡余额"),
        "after_paid_balance" : fields.float("after_paid_banalce",readonly = True,digits_compute = dp.get_precision('ktv_fee'),help = "付款后卡余额"),
    ...
    def create(self,cr,uid,vals,context={}):
      ...
      '''
      pool = self.pool
      member = pool.get('ktv.member').browse(cr,uid,vals['member_id'])
      #计算会员卡积分设置
      the_points = pool.get('ktv.points_config').calculate_room_fee_points(cr,uid,vals['total_paid_fee'])
      vals.update({
