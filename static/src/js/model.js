...
openerp.ktv_sale.model = function(erp_instance) {
    ...
    reset_display_rooms : function(critial){
      ...
      return self.pull_rooms_with_fee_info(critial).pipe(function(result) {
		...
		_refresh_app_data: function() {
			...
			}),new erp_instance.web.Model('res.users').query().all().pipe(function(ret){
        self.get('users').reset(ret);
      })).then(function() {
				self.ready.resolve();
			});
		},
    ...
    //根据room_id获取当前显示的room信息
    get_room_by_id : function(room_id){
      return this.get('display_rooms').get(room_id);
    },
		//根据包厢状态返回包厢数组
    //@param r_states array 状态列表
		get_rooms_by_state: function(r_states) {
      if(!_.isArray(r_states))
        r_states = [r_states]
      search = [['state','in',r_states]];
	...
	model.BaseRoomOperate = Backbone.Model.extend({
		...
		defaults: {
			...
			"on_credit_fee": 0.0,
			"free_fee": 0.0,
      "discounter_room_fee_discount_rate" : 100.00,
      "discounter_room_fee_discount_fee" : 0.0,
		...
		initialize: function(attrs) {
			...
			var events = "change:sales_voucher_fee change:member_card_fee change:credit_card_fee";
			this.on(events, this._re_calculate_cash_fee, this);
			this.on("change:act_pay_cash_fee", this._calculate_change_fee, this);
      //用户优惠发生变化时,重新计算打折金额
      this.on('change:on_credit_fee change:free_fee change:discounter_room_fee_discount_rate',this._re_calculate_discount_fee,this);
		},
    _re_calculate_discount_fee : function(){
      if(!this.get('discounter_id'))
        return;
      var on_credit_fee = this.get('on_credit_fee');
      var free_fee = this.get('free_fee');
      var discounter_room_fee_discount_rate  = this.get('discounter_room_fee_discount_rate');
      var total_fee = this.get('total_fee');
      //取消原会员打折,打折卡打折
      //先计算员工打折
      var total_discount_fee = (100.0 - discounter_room_fee_discount_rate)*total_fee/100.0;
      var total_after_discount_fee = total_fee - total_discount_fee;
      this.set({
        'discount_card_room_fee_discount_rate'  : 100,
        'discount_card_room_fee_discount_fee'   : 0.0,
        'member_card_room_fee_discount_rate'    : 100,
        'member_card_room_fee_discount_fee'     : 0.0,
        'total_discount_fee'                    : total_discount_fee, 
        'discount_rate'                         : discounter_room_fee_discount_rate, 
        'total_after_discount_fee'              : total_after_discount_fee,
        'total_after_discount_cash_fee'         : 0.0
      },{silent : true});

      this._re_calculate_cash_fee();
		...
		_re_calculate_cash_fee: function() {
			...
			this.set({
				...
				"act_pay_cash_fee": total_after_discount_cash_fee,
				"cash_change": 0
			});
		},
		//计算找零金额
	...
	model.RoomScheduled = model.BaseRoomOperate.extend({
		...
		initialize: function(attrs) {
			...
			this.on("change:scheduled_time change:create_date", this._set_context_datetime, this);
		},
		_set_context_datetime: function() {
			if (this.get("scheduled_time")) this.set({
				context_scheduled_time: erp_instance.web.str_to_datetime(this.get('scheduled_time')).toString('yyyy-MM-dd HH:mm')
			});
			if (this.get("input_time")) this.set({
				context_input_time: erp_instance.web.str_to_datetime(this.get('input_time')).toString('yyyy-MM-dd HH:mm')
			});
		},
		export_as_json: function() {
  ...
  //预定清单
  model.RoomScheduledList = Backbone.Collection.extend({
    model : model.RoomScheduled,
    initialize : function(models,options){
      Backbone.Collection.prototype.initialize.apply(this,arguments);
      this.on('reset',this._calculate_field,this);
    },
    _calculate_field : function(){
      this.each(function(e){
        e._set_context_datetime();
      });
    }
  });
	//开房对象
  ...
  //收银交接单
  model.CasherShiftReport = Backbone.Model.extend({
		initialize: function(attrs) {
			Backbone.Model.prototype.initialize.apply(this, arguments);
			this.on("change:print_datetime change:start_datetime change:end_datetime", this._set_context_datetime, this);
		},
		_set_context_datetime: function() {
			if (this.get("print_datetime")) this.set({
				context_print_datetime: erp_instance.web.str_to_datetime(this.get('print_datetime')).toString('yyyy-MM-dd HH:mm')
			});
			if (this.get("start_datetime")) this.set({
				context_start_datetime: erp_instance.web.str_to_datetime(this.get('start_datetime')).toString('yyyy-MM-dd HH:mm')
			});
			if (this.get("end_datetime")) this.set({
				context_end_datetime: erp_instance.web.str_to_datetime(this.get('end_datetime')).toString('yyyy-MM-dd HH:mm')
			});
		...
		push: function() {
			var json_obj = this.toJSON();
			return new erp_instance.web.Model('ktv.casher_shift_report').get_func('confirm')(json_obj);
		}
  });

  //日营业额统计报表
  model.DailyReport = Backbone.Model.extend({
		initialize: function(attrs) {
			Backbone.Model.prototype.initialize.apply(this, arguments);
			this.on("change:print_datetime change:start_datetime change:end_datetime", this._set_context_datetime, this);
		},
